...
$DOCKER_CMD run -t -v "$PWD:/data" -v "$PWD/../truck.lua:/opt/truck.lua" ghcr.io/project-osrm/osrm-backend osrm-extract -p /opt/truck.lua --algorithm mld /data/us-latest.osm.pbf --threads $THREADS 2>&1 | while IFS= read -r line; do
